...
export default function QRScanner() {
    ...
    if (!qrLock.current && data) {
      ...
      try {
        const response = await axios.get(
          `https://17cb-2402-4000-13e3-9df0-bd8b-d170-390e-b908.ngrok-free.app/api/${data}`
        );
        setRowData(response.data.vehiclesDto);

        router.push(
          `/result?license_plate_no=${response.data.vehiclesDto.license_plate_no}&vehicleType=${response.data.vehiclesDto.vehicle_type}&fuelType=${response.data.vehiclesDto.fuel_type}&availableFuelCapacity=${response.data.vehiclesDto.availableFuelCapacity}&vehicleId=${response.data.vehiclesDto.vehicleId}&userId=${response.data.vehiclesDto.userAccount.userId}`
  ...
  const handlePumpFuel = async() => {
    if (fuelAmount === "") {
      ...
      try {
        const response = await axios.get(
          `https://17cb-2402-4000-13e3-9df0-bd8b-d170-390e-b908.ngrok-free.app/api/${fuelAmount}`
        );
        setRowData(response.data.vehiclesDto);
